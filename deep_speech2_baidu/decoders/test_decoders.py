...
class TestDecoders(unittest.TestCase):
    def setUp(self):
        ...
        self.probs_seq = [self.probs_seq1, self.probs_seq2]

    def test_greedy_decoder(self):
        bst_result = [
            decoder.ctc_greedy_decoder(probs_seq, self.vocab_list) for probs_seq in self.probs_seq
        ]
        self.assertEqual(bst_result, self.greedy_result)

    def test_beam_decoder(self):
        bst_result = [
            decoder.ctc_beam_search_decoder(probs_seq, self.beam_size, self.vocab_list) for probs_seq in self.probs_seq
        ]
        bst_result = [result[0][1] for result in bst_result]

        self.assertEqual(bst_result, self.beam_search_result)

    def test_beam_batch_decoder(self):
        bst_result =\
            decoder.ctc_beam_search_decoder_batch(self.probs_seq, self.beam_size, self.vocab_list, num_processes=2)
        bst_result = [result[0][1] for result in bst_result]
        self.assertEqual(bst_result, self.beam_search_result)
